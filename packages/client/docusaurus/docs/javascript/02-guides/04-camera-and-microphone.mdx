...
The initial status of the camera is `undefined`, which will be updated with the default backend settings once the call data is loaded.
It's also possible to set the default status from the outside if you don't care about the backend settings.
...
The initial status of the microphone is `undefined`, which will be updated with the default backend settings once the call data is loaded.
It's also possible to set the default status from the outside if you don't care about the backend settings.
