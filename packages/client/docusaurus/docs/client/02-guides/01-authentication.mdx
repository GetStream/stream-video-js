...
that your App ID has both Chat and Video enabled.

For security reasons, we recommend that you generate the JWT on your backend server as generating
...
Just keep in mind to not expose your secret or your token to the public.
...
- `user`: An object that defines the user you want to connect. It should match the following type [`User`](../04-call-engine/User.md)
...
Our React Video SDK expose a utility hook that can make this process easier.
...
However, anonymous users need a call token to join a call. Compared to the regular user tokens,
the tokens issued for anonymous users must have additinal claims and constraints:
...
Additionally, the guest users can open a WebSocket connection if the right permissions are configured.

Guest users can only be created client-side and in order to be able to create a guest user, you need to fetch an "anonymous" user token first.
...
// connect the guest user to the backend
...
const anonToken: string: = 'the-anonymous-token';
