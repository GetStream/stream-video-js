 ...
 * matches what `useI18n` did before this module existed, so a component rendered outside the
 * provider keeps showing correct English instead of crashing. Chat's React Native SDK throws
 * instead; hence core exposes this as a factory rather than one exported context.
 */
 ...
 *
 * The name is `useI18n` rather than core's `useTranslationContext` because ~500 call sites across
 * the video SDKs already import it; renaming them is churn with no payoff.
 *
 * @example
 * const { t } = useI18n();
 * const label = t('common.join.label', 'Join');
 */
