...
Works great in pair with the [`Array.sort`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/sort) API.
:::
...
- `speakerLayoutSortPreset`: A preset specifically designed for the [`'default'` call type](../configuring-call-types/#default), optimizing participant sorting for speaker layout view.
- `livestreamOrAudioRoomSortPreset`: A preset tailored for the [`'livestream'`](../configuring-call-types#livestream) and [`'audio_room'`](../configuring-call-types#audio-room) call types, ensuring optimal participant sorting in livestream or audio room scenarios.
...
by using our SDKs `CallTypes` registry.
