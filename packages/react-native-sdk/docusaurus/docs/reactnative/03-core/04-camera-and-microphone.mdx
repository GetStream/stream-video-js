...
The audio by default streams through your earpiece/bluetooth speaker in all the cases.
...
InCallManager allows you to start/stop the audio output on the device speaker through:
...
InCallManager doesn't allow you to change the output device if you have multiple devices. However, you can turn on the speaker and route the audio through speaker, using:

```tsx
import InCallManager from 'react-native-incall-manager';

// For Android
InCallManager.setSpeakerphoneOn(true); // Pass true for on, and false for off.

// To force speaker phone on/off on both android and iOS.
InCallManager.setForceSpeakerphoneOn(true); // Pass true for on, and false for off. Once off audio is always routed through earpiece.
```
