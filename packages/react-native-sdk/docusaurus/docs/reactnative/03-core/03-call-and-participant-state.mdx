...
and connected to our backend: [Load Call](../../joining-and-creating-calls/#load-call).
...
The `StreamCall` component is a context provider that makes the call state available to all child components.
The `useCall` hook returns the `Call` instance that is registered with `StreamCall`. You need the `Call` instance to initiate API calls.
...
| `useCallSessionParticipantsByUserId` | The participants of the call session, mapped by the user_id property.                                         |
