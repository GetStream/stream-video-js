...
The normal user experience in a ringing app when a user receives a call is to show a notification. The user can then interact with the notification to accept or reject the call. In this guide, we explain how to set up your react-native app to get push notifications from Stream for the incoming calls that your user will receive.
...
When a user taps on the push notification, if the JS engine is not ready, we should still be able to navigate to the screen that shows the active call. We can do this by the ability to [navigate without the navigation prop in react-navigation library](https://reactnavigation.org/docs/navigating-without-navigation-prop/).

Following is an example implementation of a utility file that has helpers to statically navigate in the app:
...
Also, importantly set the `navigationRef` in your navigation container as shown below:
...
Following is an example implementation of the screen that shows the active call. It is set up to accept the incoming call once a call-id for the incoming call is received.
