...
```bash title=Terminal
yarn add react-native-webrtc react-native-device-info react-native-incall-manager react-native-svg @react-native-community/netinfo @notifee/react-native
```
...
- `Privacy - Microphone Usage Description` - "`StreamAudioRoomExample` app requires microphone access to capture and transmit audio"
  ...
  return (
    <StreamVideo client={client} language="en">
      <StreamCall call={call}>
        <SafeAreaView>
          <Text style={{ fontSize: 30 }}>Ready to render Audio room</Text>
        </SafeAreaView>
      </StreamCall>
    </StreamVideo>
  );
...
export default function App() {
  ...
  return (
    <StreamVideo client={client}>
      <StreamCall call={call}>
        // highlight-next-line
        <AudioRoom onClose={() => setCall(undefined)}/>
      </StreamCall>
    </StreamVideo>
  );
}
...
export default function AudioRoom({ onClose }: { onClose: () => void }) {
  ...
  // except for DescriptionPanel component we will put every other panels in todo
  const ParticipantsPanel = () => (
    <Text style={{ fontSize: 20 }}>ParticipantsPanel TODO</Text>
  );

  const ControlsPanel = () => (
    <Text style={{ fontSize: 20 }}>ControlsPanel TODO</Text>
  );
  const PermissionRequestsPanel = () => (
    <Text style={{ fontSize: 20 }}>PermissionRequestsPanel TODO</Text>
  );
}
```

:::note
All state for a call is available in `call.state`. In the example above, we observe if the call has been left using the `useCallCallingState` hook and if so, close the active room component. The [Call & Participant State](../../core/call-and-participant-state/) guide explains the available state objects in detail.
:::
...
In this component, we showed the title, description and the number of participants by using the metadata inside the call object. It also contains a close button for the component.
