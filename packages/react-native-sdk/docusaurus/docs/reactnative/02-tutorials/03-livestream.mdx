...
Your backend typically generates the `user` and `token` on signup or login.
...
To create the first call object, specify the call type as **livestream** and provide a unique `callId`. The `livestream` call type comes with default settings that are usually suitable for live streams, but you can customize features, permissions, and settings in the dashboard. Additionally, the dashboard allows you to create new call types as required. For more information, see the [Call Types](../../core/configuring-call-types/) docs.
...
In `App.tsx` replace the `Livestream` component with the following code:
