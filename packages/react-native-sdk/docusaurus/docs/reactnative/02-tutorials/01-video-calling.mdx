...
In order to be able to communicate with Stream's back-end video service, you will need to gather the following values and you can store them either in the environment variables of your project or somewhere else. Make sure it is easily accessible to be used later in the application:
...
- Show the info about the participants in a list view with name, status of media streaming.
- Show available options within the call like Pin participant, enable/disable audio/video, etc.

![Video Call Pariticipants Info View](../assets/02-tutorials/video-call-participants-info-view.png)
