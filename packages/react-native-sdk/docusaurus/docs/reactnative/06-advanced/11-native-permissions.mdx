...
In this guide, we will create a hook to manage the native permissions required for the app to function correctly.
This hook will be used to sync the app's permissions with the Stream Video SDK.

We will ask for relevant permissions after the parent screen is mounted and when the app state changes to the foreground.
This ensures the latest permissions are synced with the Stream Video SDK, that way the SDK will subscribe to
audio/video devices in preparation for a call.
...
In order to sync the permissions when the app comes back to the foreground, we will add a listener to the app state change event.
This will ensure that the permissions are always in sync when the user comes back to the app from the background after changing the permissions in the app's settings screen.
