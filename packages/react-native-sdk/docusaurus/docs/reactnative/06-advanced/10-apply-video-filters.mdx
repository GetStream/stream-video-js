...
This library adds the required native module for processing the video stream and applying your desired video filter.
...
:::note
`BitmapVideoFilter` is less performant than a normal video filter that does not use bitmaps due to the overhead of certain operations, like *YUV <-> ARGB* conversions.
:::
...
### Step 2 - Create a Native Module in your app

Now you have to add a method in your app to register this video filter to the `@stream-io/video-filters-react-native` library. For this, follow the official React Native documentation to create a new [Android Native module](https://reactnative.dev/docs/native-modules-android). In that native module, add a new method to add the filter to the `ProcessorMap`. For example:
...
export const useCustomVideoFilters = () => {
  ...
  const applyGrayScaleFilter = useCallback(() => {
    ...
    (call?.camera.state.mediaStream as MediaStream | undefined)
      ?.getVideoTracks()
      .forEach((track) => {
        track._setVideoEffect('grayscale'); // set this filter
      });
...
Below is a preview of the applied video filter:
![Preview of the grayscale video filter](../assets/06-advanced/10-apply-video-filters/preview-blur-filter.png)

:::note
When using a custom filter along with the built-in filters, always call the `disableAllFilter` method from the `useBackgroundFilters()` hook so that the hook is informed if the built-in filters have been disabled.
:::