...
import IncomingCallView from '../assets/06-advanced/02-chat-integration/incoming-call-view.png';
import OutgoingCallView from '../assets/06-advanced/02-chat-integration/outgoing-call-view.png';
import ActiveCallView from '../assets/06-advanced/02-chat-integration/active-call-view.png';
...
The `chatButton` prop is an object that takes up an `onPressHandler`, which is responsible to perform an action when the button is pressed. In our case, it's the opening of the `ChatScreen`.

![Preview of the call controls view with chat button](../assets/06-advanced/02-chat-integration/call-controls-view-chat.png)
...
![Preview of the unread count in call controls view](../assets/06-advanced/02-chat-integration/call-controls-view-chat-unread-count.png)
...
<ImageShowcase
  items={[
    ...
    {
      image: ActiveCallView,
      caption: 'Active Call View',
      alt: 'Active Call View',
    },
  ]}
/>
...
### Step 5: Creating the Call Panel

Our application should intercept to the states of the call and show the appropriate UI for the call.

The following states can exist for a call:

- Ringing - This is when we show the incoming or the outgoing call view to the user as a new call is received or sent.
- Joined - The call is joined. This is when we can show the Call UI.
- Joining - This is when we can show a activity loader until when the call is joined.

To intercept this states, we use the `useCallCallingState` hook provided from the SDK.

We can create the call panel as follows:

```tsx title="src/components/CallPanel.tsx"
import {
  CallingState,
  CallControlsView,
  CallContentView,
...
Here, we use the [`IncomingCallView`](../../ui-components/call/incoming-call-view/), [`OutgoingCallView`](../../ui-components/call/outgoing-call-view/) to show the incoming and outgoing call view when the call is in ringing mode. We show the [`OutgoingCallView`](../../ui-components/call/outgoing-call-view/) when the call is created by me, and the [`IncomingCallView`](../../ui-components/call/incoming-call-view/) when the call is not created by me and is received from someone else. To do that we use the following snippet above using the `useCall` hook:
...
<ImageShowcase
  items={[
    {
      image: OutgoingCallView,
      caption: 'John Calls Alice',
      alt: 'Outgoing Call View',
    },
    {
      image: IncomingCallView,
      caption: 'Alice Receives a call from John',
      alt: 'Incoming Call View',
    }
  ]}
/>

To show the joined call UI, we use the [`CallContentView`](../../ui-components/call/call-content-view/) and the [`CallControlsView`](../../ui-components/call/call-controls-view/) component from the SDK.

![Preview of the Active call screen](../assets/06-advanced/02-chat-integration/active-call-view.png)

Now, we pass the `CallPanel` created above as a child of the `Calls` component:
...
Finally, when you press on the call button, an outgoing call be sent to the participant, which can be accepted or rejected using the Incoming call view screen. On accepting, the call is joined the participants and the call controls are available.
