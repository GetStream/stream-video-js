...
One of the crucial functionalities of a video or audio calling application is to keep the call alive in the background. In this page, we focus on what must added in your app to support this. After enabling, the user of your app will notice that the call is kept alive even if the app goes to background as they will still hear the remote audio streams while the app is kept in the background.
...
The way to keep audio alive in the background is to enable the Voice over IP background mode. When you enable this capability, your app's audio playback will continue to play when users lock their iOS device or switch to another app. In Xcode: Open `Info.plist` file and add `Voice over IP` in `UIBackgroundModes`. By editing this file with a text editor, you should see:
