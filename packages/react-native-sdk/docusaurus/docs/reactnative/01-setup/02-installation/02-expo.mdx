...
The `@stream-io/video-react-native-sdk` handles the [Add Stream Video SDK's setup method](../react-native/#add-stream-video-sdks-setup-method) automatically and adds the appropriate android permissions such as `POST_NOTIFICATIONS`, `FOREGROUND_SERVICE`, `FOREGROUND_SERVICE_MICROPHONE`, `BLUETOOTH`, `BLUETOOTH_ADMIN` and `BLUETOOTH_CONNECT` to their respective files.

```js title=app.json {5-13}
{
  "expo": {
    ...
    "plugins": [
