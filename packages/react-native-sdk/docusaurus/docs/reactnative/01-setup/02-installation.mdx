...
```bash
yarn add @stream-io/video-react-native-sdk react-native-webrtc react-native-callkeep react-native-device-info react-native-incall-manager react-native-svg text-encoding-polyfill react-native-url-polyfill react-native-get-random-values intl-pluralrules @react-native-community/netinfo @notifee/react-native
```
...
- `react-native-url-polyfill` is a polyfill for the `Encoding Living Standard API` for the Web. This dependency allows encoding and decoding of textual data to and from Typed Array buffers for binary data in JS.
- `react-native-get-random-values` is used for `crypto.getRandomValues` polyfill to generate uuids.
...
### Initial app setup: encoding and polyfills
