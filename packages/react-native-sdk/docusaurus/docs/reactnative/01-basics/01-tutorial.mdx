...
We will create the call using the `client.call` method and pass it to the `StreamCall` component, that will provide the call to its children and all other SDK components.
...
`StreamCall` component has also a `callCycleHandlers` prop of type object that expects `onCallJoined`, `onCallHungUp` handlers (among other handlers) to deal with transitions between the call states.
...
import { LobbyView, ActiveCall } from '@stream-io/video-react-native-sdk';
