...
`StreamVideo` component has a `callCycleHandlers` prop of type object that expects `onCallJoined` and `onCallHungUp` handlers to deal with the case when the call is active and when the call is hung up. These handlers can be used to navigate to particular screens depending on the state.
