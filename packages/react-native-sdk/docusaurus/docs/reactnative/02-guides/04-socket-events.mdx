...
As soon as you create a `VideoClient`, we connect to a signaling WebSocket instance which is responsible for all call-based events - primarily for the call connection flow, codec negotiation and similar, but also about participants and their mute or audio states.
...
All of these events are processed internally, in the `VideoClient`, but you can listen to them to build custom behavior or UI.
