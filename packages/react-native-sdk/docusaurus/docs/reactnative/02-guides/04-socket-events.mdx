...
A great part of our React Native Video SDK are real time updates based on socket events from the server. There are two sets of WebSocket connections and types of events that our SDK receives and consumes:
...
All of these events are processed internally to update the state of our lower-level client and UI Components. If you're using our default UI components, based on the events and the [call engine](./03-call-engine.mdx) state, we launch our out of the box `ActiveCall` that handles incoming, outgoing and call starting flows.
...
As soon as you create a `StreamSFUClient`, we connect to a signaling WebSocket instance which is responsible for all call-based events - primarily for the call connection flow, codec negotiation and similar, but also about participants and their mute or audio states.
...
All of these events are processed internally, in the `StreamVideoClient`, but you can listen to them to build custom behavior or UI.

Once you're ready to stop listening to events, be it when leaving a call or when you disable custom features, you can video the listener in a similar manner to the coordinator WebSocket:
