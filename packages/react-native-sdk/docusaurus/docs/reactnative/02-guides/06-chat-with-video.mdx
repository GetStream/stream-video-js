...
`StreamVideoClient` expects API Key as the first arguments. It also expects `options` and Call config object for type `ring`(`CALL_CONFIG.ring`).
...
- `onHangupCall` handler to deal with the case when the call is hunged up.
...
To render the incoming call view, you can import `IncomingCallView` from the Reatc Native Video SDK and use it in the Screen/View of your choice.
