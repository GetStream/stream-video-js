...
- `onCallJoined` handler to deal with the case when the call is active.
- `onCallHungUp` handler to deal with the case when the call is hung up.
- `onCallIncoming` handler to deal with the case when there's an incoming call.
- `onCallOutgoing` handler to deal with the case when there's an outgoing call.
- `onCallRejected` handler to deal with the case when an incoming call is rejected.
  ...
  return (
    <StreamVideo
      ...
      callCycleHandlers={{
        ...
        onCallHungUp: () => console.log("Call Hanged up"),
