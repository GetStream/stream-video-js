...
Network conditions are not always perfect so instead of playing guessing games as to why is someone's video so blurry our default [`Participant`](../../ui-components/participants/participant/) component comes equipped with connection quality indicator which provides general information about that participants network conditions in four states:
  ...
  return (
    <View>
      {participants.map((participant) => {
        // Code for your custom participant component
        return (
          <View>
            {/* ParticipantLabel is rendered as a sub-component here */}
            <CustomParticipantUI participant={participant} />
          </View>
        );
      })}
    </View>
