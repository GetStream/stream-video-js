 ...
 * stays here, because it is generated from this package's own `t()` call sites. That split is why
 * core's helpers are generic over the catalog rather than driven by module augmentation — two
 * catalogs have to be able to coexist in one TypeScript program, which a monorepo typechecking the
 * web and React Native SDKs in one pass does.
 *
