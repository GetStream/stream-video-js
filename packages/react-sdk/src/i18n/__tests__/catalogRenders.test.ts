 ...
 * by the same generator run and living under `__tests__` so it never reaches the published build.
 * Ported from the Chat React SDK.
 */
...
describe('translation catalog renders', () => {
  ...
  it('renders every plural key at each count without leaking the key or a placeholder', async () => {
    ...
    for (const base of pluralBases) {
      ...
      // Without this, every assertion above would still pass on a runtime whose `Intl.PluralRules`
      // has no English data: `_other` would answer for every count and nothing would say so.
  ...
  it('renders English for keys a partial dictionary omits', async () => {
    const translated = 'callControls.toggleAudioButton.mic.title';
    const omitted = 'callControls.toggleVideoButton.video.title';

    const i18n = new Streami18n({ logger: () => {} });
    await i18n.init();
    i18n.registerTranslation('de', { [translated]: 'Mikrofon' });
    await i18n.setLanguage('de');

    const render = i18n.t as unknown as Render;

    expect(render(translated, copyOf(translated))).toBe('Mikrofon');
