...
In this tutorial, we are building a video call application using the Stream Video React SDK. We will be implementing a "Meeting Room" scenario, where users can join and leave the call independently, without causing the call to be interrupted. The topics we are covering:
...
On another important context provider is `MediaDevicesProvider`. It takes care of registering media peripherals that would be later used in the call.
...
The theme toggle button will be positioned with `position: fixed` on every screen:
