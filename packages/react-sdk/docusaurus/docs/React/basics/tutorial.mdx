...
This tutorial replicates a creation of an existing sample application. We have made [the source code](https://github.com/GetStream/stream-video-js/tree/main/sample-apps/react/stream-video-react-tutorial) available for your convenience.
...
The only thing we will do here is that we import the stylesheet provided by Stream's SDK into `src/style/index.css` as a separate CSS layer and will remove its import from `src/App.tsx`:
...
Please find all the custom stylesheets [in the reference application source](https://github.com/GetStream/stream-video-js/tree/main/sample-apps/react/stream-video-react-tutorial/src/style).
