...
The [`ReactionsButton`](TODO) and [`ParticipantReaction`](TODO) components support reactions out-of-the-box, but for advanced use-cases you can also build your own reaction system.

:::

### Sending reactions

You can send a reaction using the [`sendReaction`](../../call-engine/Call/#sendreaction) method of a `Call` instance.
...
If you're using the [participant state](../../guides/call-and-participant-state/#observe-participant-state) for receiving reactions, you can also clear the latest reaction using the [`resetReaction`](../../call-engine/Call/#resetreaction) method:
...
You can use the [`sendEvent`](../../call-engine/Call/#sendevent) method of the `Call` instance to send custom events:
