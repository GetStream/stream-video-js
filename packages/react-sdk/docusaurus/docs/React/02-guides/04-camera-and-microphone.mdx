...
It could happen that the user did not yet enable their camera and microphone settings in the browser. You could want to present them with a custom message or component to notify them that these settings need to be accepted. One elegant way to do this is to leverage the `VideoPreview` component.

```typescript

export const EnableBrowserSettings = () => {
  return (
    <div>
      <h2>
...
export const App = () => {
  ...
  const {  isAwaitingPermission, hasBrowserMediaPermissions } = useBrowserMediaPermissions();

  useEffect(() => {
    if(isAwaitingPermission === false && hasBrowserMediaPermissions){
      setPermissionsEnabled(true)
    }

  }, [isAwaitingPermission, hasBrowserMediaPermissions])

  return (
    <StreamCall call={call}>
      <VideoPreview
          StartingCameraPreview={() => permissionsEnabled ? <SomePlaceholderComponent /> : <EnableBrowserSettings />  }
          VideoErrorPreview={() => permissionsEnabled ? <SomePlaceholderComponent /> : <EnableBrowserSettings />}
        />
    </StreamCall>
  );
};
