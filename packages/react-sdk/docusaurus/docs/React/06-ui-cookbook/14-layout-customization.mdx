...
We highly recommend using layout components provided by the SDK as it is possible to modify the `ParticipantViewUI` in these call layouts. You can follow [this guide](./13-custom-call-layout.mdx) to build your own call layout.
...
- component which accepts `participant` as a prop (use type `ParticipantViewUIProps`)
