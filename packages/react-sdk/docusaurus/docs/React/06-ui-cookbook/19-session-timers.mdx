...
await client.video.updateCallType('appointment', {
  grants: {
    specialist: [
      VideoOwnCapability.SEND_AUDIO,
      VideoOwnCapability.SEND_VIDEO,
      // Specialist can change the maximum duration:
      VideoOwnCapability.CHANGE_MAX_DURATION,
    ],
    patient: [VideoOwnCapability.SEND_AUDIO, VideoOwnCapability.SEND_VIDEO],
  },
  settings: {
    limits: {
      max_duration_seconds: 1800, // 1800 seconds = 30 minutes
    },
  },
});
...
The `specialist` user role that we created has the `CHANGE_MAX_DURATION`
capability, which allows a user to change the duration of a call. Let's add a
component that updates the call settings and extends a session by the specified
number of seconds:
...
const ExtendSessionButton = ({ duration }) => {
  ...
  const canExtend = useHasPermissions([VideoOwnCapability.CHANGE_MAX_DURATION]);
    ...
    <button
      ...
      onClick={() => {
        call.update({
          settings_override: {
            limits: {
              max_duration: settings.limits.max_duration + duration,
            },
          },
        });
      }}
