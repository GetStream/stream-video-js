...
import DefaultParticipantLabelMuted from '../assets/ui-cookbook/03-custom-label/default-participant-label-muted.png';
import DefaultParticipantLabelConnectionDominantSpeaker from '../assets/ui-cookbook/03-custom-label/default-user-label-connection-dominant-speaker.png';

There may be a plethora of designs for label that is displayed above the participant video. The SDK's [default label shows](todo docs link to section about DefaultParticipantViewUI):
...
Our custom label will **include a human-readable call join time** in addition to what the design requirements were. To achieve this, we will need to override the whole [`DefaultParticipantViewUI`](todo docs link to section about DefaultParticipantViewUI) component. You can learn more about `ParticipantViewUI` customizations in [the participant view customizations guide](../participant-view-customizations).

To retrieve the data about the participant, whose view we are about to render, we use [`useParticipantViewContext`](todo docs link) context consumer hook.
