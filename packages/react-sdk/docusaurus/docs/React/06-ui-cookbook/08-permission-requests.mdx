...
The [`ToggleVideoPublishingButton`](/TODO), [`ToggleAudioPublishingButton`](/TODO) and [`ScreenShareButton`](TODO) components can send permission requests, you only need to follow this guide if you want to build your own UI for this.

The [`PermissionRequests`](TODO) component can receive permission requests, you only need to follow this guide if you want to build your own UI for this.
  ...
  return (
    <StreamTheme>
      <StreamVideo client={client}>
        <StreamCall
          callId={'test-call-permission-requests'}
          callType={'audio_room'}
          data={{ create: true }}
          autoJoin
        >
...
To follow along with the guide, don't forget to [turn off backstage mode](../../guides/call-lifecycle/#backstage-mode) so others can join:
...
- To send the request we are using the [`requestPermissions` method](../../call-engine/Call/#requestpermissions) of the `Call` instance.
- We use the [`useHasPermissions`](../../call-engine/hooks-and-contexts/#usehaspermissions) hook to be notified when the request was approved. Alternatively you can subscribe to the [`call.permissions_updated` event](../../advanced/events/).
...
- We use the [`useCallPermissionRequest`](../../call-engine/hooks-and-contexts/#usecallpermissionrequest) hook to be notified about permission requests
- A permission request can be accepted with the [`grantPermissions`](../../call-engine/Call/#grantpermissions) method of the `Call` instance
- A permission request can be rejected with the [`revokePermissions`](../../call-engine/Call/#revokepermissions) method of the `Call` instance. This method can also be used to revoke permissions later in the call.
