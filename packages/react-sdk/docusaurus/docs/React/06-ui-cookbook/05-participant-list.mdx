# Building Custom Participants List layout
...
1. Declaratively using the `<StreamMeeting />` component.
2. Imperatively using the `StreamVideoClient` API methods (advanced usage).
...
Behind the scenes, the `<StreamMeeting />` component will set up and join a new [Call](./Call.md) instance
and it will expose down in the component tree through a React Context the [CallState](./CallState.md) object.

You can read more about this mechanism as part of our [React Bindings and Hooks documentation](./ReactSDK.md).

# Building the Layout

Now that we have successfully set up the React Video SDK and joined the call, we can start building our custom layout.
For that purpose, we are going to utilize some of the fundamental React Video SDK components and hooks.

- `<ParticipantBox />` - A component that renders a video stream of a participant.
- `<CallControls />` - A component that renders the call controls (mute/unmute audio, mute/unmute video).
...
- `useDominantSpeaker()` - A hook that returns the current dominant speaker in the call.
...
The React Video SDK is built on top of the data model that our [JavaScript Video SDK](./CallEngine.md) exposes.
