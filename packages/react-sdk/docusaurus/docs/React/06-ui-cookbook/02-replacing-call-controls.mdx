...
:::note
The React SDK exports a pre-built component [`CallControls`](https://github.com/GetStream/stream-video-js/tree/main/packages/react-sdk/src/components/CallControls/CallControls.tsx). If it does not meet all the requirements, we encourage everybody to assemble their own `CallControls` component.
:::
...
We will need a call accept button when building app that makes use of [ring call workflow](../../guides/call-lifecycle#ring-call). To accept a call we just invoke `call.join()`. So the minimal call accept button could look like this:
...
export const CustomCancelCallButton = ({ reject }: CustomCancelCallButtonProps) => {
  const call = useCall();
  return (
...
export const CustomToggleAudioPublishingButton = () => {
  const call = useCall();
  // the API to control the audio enablement in an active call
  ...
  const handleClick = useCallback(async () => {
    if (isAudioMute) {
      // Make sure to enable the initial state
...
export const CustomToggleVideoPublishingButton = () => {
  const call = useCall();
  // the API to control the video enablement in an active call
  const { publishVideoStream, stopPublishingVideo, setInitialVideoState } =
    useMediaDevices();
  const localParticipant = useLocalParticipant();
  // Check, whether the video input stream is already sent to Stream's server
...
export const CustomToggleVideoPreviewButton = () => {
  ...
  const { toggleInitialVideoMuteState, initialVideoState } =
    useMediaDevices();
...
Screensharing involves sharing a video stream, which is obtained through the use of the  [`navigator.mediaDevices.getDisplayMedia()`](https://developer.mozilla.org/en-US/docs/Web/API/MediaDevices/getDisplayMedia) call. To simplify this process, the SDK exports a utility function [`getScreenShareStream`](todo docs link), that retrieves the `MediaStream` object for us. Additionally, we utilize a screenshare-specific hook called [`useHasOngoingScreenShare`](../../call-engine/hooks-and-contexts#usehasongoingscreenshare) which informs us whether any call participant is already sharing their screen.
