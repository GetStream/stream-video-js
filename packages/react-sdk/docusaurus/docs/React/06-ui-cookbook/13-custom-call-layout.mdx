---
id: custom-call-layout
title: Custom Call Layout
---
...
1. Declarative, using the [`<StreamCall />`](TODO#streamcall) component.
...
export const App = () => {
  ...
  return (
    <StreamVideo client={client}>
      <StreamCall callId={callId} callType="default" data={{ create: true }}>
        <SpeakerView />
      </StreamCall>
    </StreamVideo>
  );
...
Behind the scenes, the `<StreamCall />` component will set up and join a new [Call](../04-call-engine/Call.md) instance,
...
You can read more about this mechanism as part of our [React Bindings and Hooks documentation](../../call-engine/01-call-engine-overview).
...
- [`<ParticipantView />`](TODO#participantView) - A component that renders a video stream of a participant.
...
export const SpeakerView = () => {
  ...
  return (
    ...
    <StreamTheme className="speaker-view">
...
In calls with large number of participants your horizontal list will grow past viewable boundaries. Each of these `ParticipantView` components will keep subscribing to the video stream and will keep playing it even if they're not visible to the user which is wasteful and CPU and bandwidth heavy. To mitigate this issue each `Call` instance contains one `ViewportTracker` instance (`call.viewportTracker`) with `observe` and `setViewport` methods. `ParticipantView` component already utilises this functionality under the hood (`observe`) so all you need to do is to call `setViewport` with your horizontal list element as an argument and you're set.
...
Now these changes will ensure that you only subscribe to the video streams you actually see on your screen. And that's it. We have successfully built our custom layout. At last, you can find the running example of this guide [here](https://github.com/GetStream/stream-video-js/sample-apps/react/cookbook-participant-list).

:::note
Tutorial does not include the styling of the component, but you can find the full code in the [SpeakerView.scss](https://github.com/GetStream/stream-video-js/blob/main/sample-apps/react/cookbook-participant-list/src/SpeakerView.scss) file.
:::
