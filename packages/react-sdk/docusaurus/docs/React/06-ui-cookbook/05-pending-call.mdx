...
Enable the initial video in [`VideoPreview`](todo docs link) component with `mediaDevicesProviderProps` prop on [`StreamCall`](todo docs link) component. In the snippet below, the `CallPanel` component is expected to render our `CustomPendingCallPanel`.
