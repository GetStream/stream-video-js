...
:::important
When implementing a ring call scenario as we do in this demo, it is important to have a good understanding of our **ring call lifecycle**. You can learn more about the topic in the [doc page dedicated to call lifecycles](../../guides/call-lifecycle#ring-call).
:::
...
Once a ring call is initiated, call members start to receive ring call events (`call.created`, `call.accepted`, `call.rejected`, `call.ended`) over the WebSocket maintained by the video client. The video client updates the [**calls pool**](../../call-engine/hooks-and-contexts#usecalls) state and [**calling state**](../../call-engine/hooks-and-contexts#usecallcallingstate) of individual affected [`Call`](../../call-engine/Call) in response to these events.

### Observing the calls pool state

The array of all [`Call`](../../call-engine/Call) objects representing created pending (not accepted, rejected, neither ended) calls is continuously updated in response to each new call creation. You can use this array of [`Call`](../../call-engine/Call) objects to display **incoming** or **outgoing calls** in your application's UI. In our app the top-level [`Video`](https://github.com/GetStream/stream-video-js/tree/main/sample-apps/react/messenger-clone/src/components/Video/Video.tsx) component observes the changes using the [`useCalls`](../../call-engine/hooks-and-contexts#usecalls) hook and re-renders the UI to reflect the changes:
...
:::note
To identify an outgoing call, use the [`Call.isCreatedByMe`](../../call-engine/Call#iscreatedbyme) flag.
:::

In our demo app, we reflect the incoming calls state in channel list. Channel preview shows buttons to accept or reject the incoming call. The outgoing call or incoming call in an active channel is represented by a [`CallPanel` component](todo doc link) floating above the chat UI:
...
export const ChannelPreviewCallControls = () => {
  ...
  // highlight-next-line
  // the Call instance is passed down from StreamCallProvider located in StreamMeeting
  const call = useCall();
...
Our [`PendingCallControls`](todo doc link) attaches the following callback to the [`CancelCallButton`](todo doc link) button that rejects the call:
...
The default callback of the [`CancelCallButton`](todo doc link) button is pure `call.leave()` without any parameters. Again the [`PendingCallControls`](todo doc link) as well as the demo's active call panel components make use of this by not overriding the default `onClick`:
...
File [`index.scss`](https://github.com/GetStream/stream-video-js/tree/main/sample-apps/react/messenger-clone/src/style/index.scss) is then imported into [`App.tsx`](https://github.com/GetStream/stream-video-js/tree/main/sample-apps/react/messenger-clone/src/App.tsx). Additionally, we add [`StreamTheme`](todo doc link) component to wrap the app in an element carrying CSS class `str-video`. This will make sure all the variables and styles are applied to the child components:
