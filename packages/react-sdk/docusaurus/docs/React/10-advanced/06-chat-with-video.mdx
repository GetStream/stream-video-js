...
In our demo app, we reflect the incoming calls state in channel list. Channel preview shows buttons to accept or reject the incoming call. The outgoing call or incoming call in an active channel is represented by a [`CallPanel` component](../../ui-components/call/pending-call-panel) floating above the chat UI:
...
Our [`PendingCallControls`](../../ui-components/call/pending-call-panel/#pending-call-controls) attaches the following callback to the [`CancelCallButton`](../../ui-components/call/call-controls/#cancelcallbutton) button that rejects the call:
...
The default callback of the [`CancelCallButton`](../../ui-components/call/call-controls/#cancelcallbutton) button is pure `call.leave()` without any parameters. Again the [`PendingCallControls`](../../ui-components/call/pending-call-panel/#pending-call-controls) as well as the demo's active call panel components make use of this by not overriding the default `onClick`:
