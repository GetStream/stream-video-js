...
The service is made available through the `StreamVideo` provider (**TODO: provide correct link to StreamVideo provider doc**). That means that all the child components of this provider can access the `StreamI18nContextValue` object by using the `useI18n` context consumer.
