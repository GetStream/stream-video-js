...
We are going to use the `@stream-io/video-filters` library that provides background blur and background replacement filters.
...
```bash
npm install @stream-io/video-filters
# or
yarn add @stream-io/video-filters
```
