...
Client events are always delivered if a user is connected to the client using the [`connectUser` method](../../call-engine/StreamVideoClient/#connectuser).
...
You can use the [`on`](../../call-engine/StreamVideoClient/#on) method of the `StreamVideoClient` instance to subscribe to client and call WebSocket events.
...
You can use the [`on`](../../call-engine/Call/#on) method of the `Call` instance to subscribe to WebSocket events belonging to a specific call.
