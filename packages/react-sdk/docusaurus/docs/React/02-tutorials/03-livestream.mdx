...
## Quickstart
In this quickstart, we'll build a live streaming experience that's similar to Twitch.

- The live stream will run on Stream's Edge network of servers around the world
- You can use ultra-low latency (WebRTC) based live streaming or HLS (slight delay, but better buffering)
- We'll show how to publish to the live stream from your browser
- Stream enables you to scale to millions of viewers
- You can have multiple active participants in the live stream

The app that we are going to build would work in two modes: Host (presenter) and Viewer (audience).

The host will be able to publish the live stream and the viewers will be able to watch the live stream.
If you want to see a fully-fledged example, check out this [sample app.](https://github.com/GetStream/stream-video-js/tree/main/sample-apps/react/livestream-app)
...
- Add Stream's React Video SDK to your project:
  - `yarn install @stream-io/video-react-sdk` or
  - `npm install @stream-io/video-react-sdk`
- Setup a Stream account at [GetStream's Dashboard](https://dashboard.getstream.io) or use your existing one
  - We will need your API key and secret

## Step 2 - Setup the host mode

Hosts should be communicating with each other in real-time. For that purpose, we would use the standard Stream Video Call component, based on WebRTC.
...
[Authenticating your Users](../../guides/01-authentication/)
:::

In the snippet below, we create a connection to the Stream Edge server for the live stream host, we join the call and we publish our audio and video to the live stream.
In this example, we use the predefined `<PaginatedGridLayout>` for the live stream.

In case you want to have a more custom layout, you can build your own. See [Building Custom Layouts.](../ui-cookbook/participant-list/)
...
The `useCall` hook provides you with the current call object. This is useful when you want to access the call object from a component that is deep in the component tree with the `<StreamMeeting />` component as a predecessor.

On the other hand, the `useIsCallBroadcastingInProgress` hook provides you with the current broadcasting state of the call. This is useful when you want to show the current state of the live stream or build custom UI components or flows based on this call state.
...
- have a `useEffect` that would run whenever the we have initiated a change in the broadcasting state of the call until the value of `isBroadcasting` toggles.
...
For the WebRTC, we are going to use the same `<StreamMeeting />` component as in the host mode. The only difference is that we are going to use an anonymous user for the viewer.
...
Alternatively, you can keep polling the backend for the current state of the call. See [Querying Calls.](../02-guides/06-querying-calls.mdx)
:::
...
export const ViewerApp = () => {
  ...
  return (
    <StreamVideo client={client}>
      <StreamMeeting call={activeCall} autoJoin={true}>
        <WebRTCLivestreamUI />
      </StreamMeeting>
    </StreamVideo>
  )
...
For the HLS-based live streaming, we are going to use a few components from the Stream Video SDK for React but, instead using a pre-built layout,
...
Having this done, we should have a fully functional live streaming experience for the viewers based on HLS.
