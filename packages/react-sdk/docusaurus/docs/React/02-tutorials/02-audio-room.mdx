...
1. **Login page (omitted)** - here we select and authenticate a user
...
:::note
Routing in the demo app is done by using 3rd party library [react-router-dom](https://reactrouter.com/en/main).
:::
...
You can store these values in your project's `.env` file. Make sure, your app retrieves these values from the `.env` file:
...
The credentials can then be passed to the `StreamVideoClient` constructor:
...
export const VideoClientProvider = ({ children }: ChildrenOnly) => {
  // retrieve the API key, user and token...
...
The token can be retrieved using `tokenProvider` function that needs to be aware of the **selected user** and the **API key**. It then builds URL for generating the token. The URL building logic is custom to a project. Note that in the example below, we are adding a new environment variable `VITE_TOKEN_PROVIDER_URL`:

```bash
# ... already listed variables go here
...
export const VideoClientProvider = ({ children }: ChildrenOnly) => {
  // retrieve the API key, user, token, tokenProvider,
  // initiate the state to hold the client instance ...

  useEffect(() => {
    if (!user) return;
    setClient(
      new StreamVideoClient({
        // omitted constructor params ...
        // highlight-next-line
        tokenProvider,
        // omitted constructor params ...
      }),
