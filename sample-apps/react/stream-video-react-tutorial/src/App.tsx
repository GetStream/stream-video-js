...
const AudioHealthBadge = () => {
  const { useAudioHealth, useAudioRoute } = useCallStateHooks();
  ...
  const micDevice = route?.inputs[0]?.name;
  const speakerDevice = route?.outputs[0]?.name;

  useEffect(() => {
    if (!route) return;
    console.info(
      '[tutorial] audioRoute → mic:',
      micDevice ?? '—',
      'spk:',
      speakerDevice ?? '—',
    );
  }, [micDevice, speakerDevice, route]);
  ...
  return (
    ...
    >
      <SideIndicator label="mic" status={micStatus} device={micDevice} />
      <SideIndicator
        label="spk"
...
const SideIndicator = ({
  ...
  <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}>
    ...
    />
    {device && <span style={{ opacity: 0.85, maxWidth: 140 }}>: {device}</span>}
  </span>
);
